...
public class BasicEvents implements Events {
	...
	/** Objects listening for an execution to fail */
	private final List<OnExecutionFailed> executionFailedListeners = new ArrayList<>();
	...
	public void hasFailed(Execution execution) {
		for (OnExecutionFailed listener : concat(executionFailedListeners, executionListeners)) {
	...
	public void onExecutionFailed(OnExecutionFailed listener) {
		executionFailedListeners.add(requireNonNull(listener, "Cannot register a null listener"));
	}
